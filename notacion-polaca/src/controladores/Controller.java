...
public class Controller {
 ...
 /*Iteramos por los nodos para asignarle la posicion en el plano*/
        arbol.getNodos().stream().forEach((nodo) -> {
            ...
            int columsToNode = impar(nodo.numFromCenter - 1);/*En que columna deberiamos posicionar al nodo*/


            levelColumnsSize *= nodo.posRelativeToRoot == 1 ? -1 : 1;
        ...
        arbol.getNodos().stream().filter((nodo) -> (nodo.getParent() != null)).forEach((nodo) -> {
            drawLine(canvas, nodo.getParent().panelPosition, nodo.panelPosition, lineWidth);
        });
        /*Luego dibujamos los circulos*/
        arbol.getNodos().stream().forEach((nodo) -> {
            drawCircle(canvas, nodo.panelPosition, radius, nodo.getTag());
        });
